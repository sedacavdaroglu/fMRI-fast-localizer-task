...
%Localizer task to isolate reading, subtraction, grasping, fixation and
%color naming related regions of interest
...
v_dist      = 68;   % v1iewing distance (cm)
...
fix_cord3 = [center-fix_r*ppd center+fix_r*ppd]; 
disp(fix_cord3);
[X,Y] = RectCenter(rect);
...
start_trigger = vbl + (waitframes - 0.5) * ifi;
...
output_list = {'subject','trial_no','type','start_trigger','time_start','time_end','word1','word2','word3','word4','word1_start','word1_end','word2_start','word2_end','word3_start','word3_end','word4_start','word4_end',...
    ...
    'fix11_start','fix11_end','fix12_start','fix12_end','block','dir'};
...
% Sentences Block:
trialNo = drawSentences(subNo,trialNo,w,fix_cord,fix_cord2,fid,start_trigger);

%Roof Colors Block:
trialNo = roofColors(subNo,trialNo,w,fix_cord,fix_cord2,fid,rect,start_trigger);

%Objects_Saccades Block:
trialNo = objectsSaccades(subNo,trialNo,w,fix_cord,fix_cord2,fix_cord3,fid,rect,start_trigger);
% 
%Motion localizer
motion(subNo,w,fix_cord,fix_cord2,fid,rect,start_trigger)
