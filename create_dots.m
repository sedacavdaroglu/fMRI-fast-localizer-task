...
design = 4; % number 1, 2, 3, or 4 see above for explanation
S1 = [7 11 16];
S2 = 5;
% S1 = [5 5 7 7 11 11 16 16]; %a vector of numbers
% S2 = [4 6 5 9 8 14 12 20]; %a single number for designs 1, 2, 3 and a vector of numbers for design 4
nrofblocks = 8; %number of repetitions of all stimuli
...
video = 1; %2 to have the stimuli presented on the screen using psychtoolbox otherwise 1
...
stimulusTime = 0.1; %duration of the stimulus
...
for block = 1:nrofblocks;
    ...
    for j = 1:length(CondMatrix);
        ...
        for repetition = 1:NrOfRepetitionsF;
            ...
            if video == 2;
                Screen('FillOval', w, stimColor,double(int16(sizesF)));
%                 Screen('DrawDots', w,centersF,radiusF,stimColor,[],2)
            ...
            if images == 2;
                ...
                imageName = strcat('dot_images\no_comp3\',int2str(F),'_',int2str(S),'_',(int2str(block)),'_1.bmp');
%                 imageName = strcat('dot_images\',int2str(F),'_1_',(int2str(block)),'.bmp');
%                 
%                 if exist(imageName, 'file') == 2
%                     imageName = strcat('dot_images\',int2str(F),'_2_',(int2str(block)),'.bmp');
%                 end;
        ...
        if images == 2;
            ...
            imageName = strcat('dot_images\no_comp3\',int2str(S),'_',int2str(F),'_',(int2str(block)),'_2.bmp');
            imwrite(imageArray, imageName);
            picturename{trialnr,1} = imageName;
        end
...
dlmwrite('dot_images\no_comp3\all_comp.txt',MatrixCondOut);
dlmwrite('dot_images\no_comp3\regr_comp.txt',regr1);
dlmwrite('dot_images\no_comp3\congr_comp.txt',regr3);
if images == 2;
    namefile = sprintf('%s\n', picturename{:});
